...
/**
 * The backend host, for every caller on the site and not only the lead forms:
 * the product catalogue and the video gallery read from it too, and each had
 * its own copy that fell back to http://localhost:5050. `LEADS_API_BASE` is
 * kept as an alias so existing importers are unaffected.
 */
export const API_BASE =
  process.env.NEXT_PUBLIC_API_URL || process.env.NEXT_PUBLIC_API_BASE_URL || DEFAULT_API_BASE;

export const LEADS_API_BASE = API_BASE;
