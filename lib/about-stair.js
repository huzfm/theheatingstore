...
/**
 * Rounds a trig-derived length to 4dp.
 *
 * Math.tan / Math.hypot / Math.asin are implementation-defined in their last
 * bits, and Node and V8-in-Chrome do not always agree. Any such value
 * interpolated raw into Timeline's inline <style> therefore serialises to a
 * different string on the server than on the client, which React reports as a
 * hydration mismatch and then re-renders the whole subtree over. 4dp is far
 * below a sub-pixel and puts both sides on the same string.
 */
 ...
 */
export const TREAD_W = round4(2 * OUTER_R * Math.tan((STEP_ANGLE / 2) * DEG));
...
const RAIL = (() => {
  ...
  return {
    // Rounded for the same reason as TREAD_W: this one reaches the stylesheet
    // as both a width and a negative half-width margin.
 ...
 * Label visibility.
 *
 * Pinned to 0 outside 60° of front, which is inside the 72° between one step
 * and the next: at the moment a year is square to the camera its neighbours
 * are already at exactly zero. That bound is the point, a year read at a steep
 * angle is compressed to the width of a few pixels and reads as a smear beside
 * the one you are meant to be looking at, not as depth. Smoothstepped so it
 * still arrives without a hard edge.
 */
export function labelFadeAt(progress, index) {
  const t = Math.min(1, Math.max(0, (facingAt(progress, index) - 0.5) / 0.5));
  return t * t * (3 - 2 * t);
}
