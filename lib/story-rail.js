import { ORIGIN, PRINCIPLES } from '@/components/sections/About/data';

/**
 * Single source of truth for the About story rail, the pinned section where
 * the origin narrative and the three principles scroll past a sticky 3D
 * heating mat.
 *
 ...
 * block is 85vh tall and the sticky panel is 100vh, so with a 60vh tail the
 * scrubbed range is 385vh and block i reaches viewport centre at
 * (i*85 + 42.5 - 50) / 385. Deriving them this way is what keeps the camera
 * state changing at the moment a paragraph lands, rather than a fifth of the
 * way past it.
 *
...
export const RAIL_BLOCKS = [
  {
    id: 'origin-1',
    at: 0.0,
    kicker: ORIGIN.eyebrow,
    // The section's title rides with the first paragraph rather than sitting
    // above the rail, the pinned panel is already holding the right column,
    // so a separate full-width heading would push the rail 40vh down the page
    // before anything happens.
    title: ORIGIN.title,
    body: ORIGIN.paragraphs[0],
  },
  {
    id: 'origin-2',
    at: 0.2,
...
export const RAIL_KEYS = [
  // Origin ¶1, "imported systems fail here". Low raking angle, cable dead.
  { at: 0.0, y: 1.15, z: 7.1, yaw: -0.34, tilt: 0.0, level: 0.0 },

  // Origin ¶2, "so they built for the conditions". Ignition: the cable
  // lights and the current starts travelling the run.
  { at: 0.2, y: 2.55, z: 5.9, yaw: -0.12, tilt: 0.05, level: 1.0 },

  // Principle 01, Experts of the Trade, near-overhead inspection framing,
  // where the cold tail and factory splice are legible.
  { at: 0.42, y: 4.7, z: 2.7, yaw: 0.18, tilt: 0.0, level: 0.95 },

  // Principle 02, Built for the Blackout, camera settles back, glow still
  // at full.
  { at: 0.64, y: 3.35, z: 4.55, yaw: 0.42, tilt: 0.04, level: 0.98 },
