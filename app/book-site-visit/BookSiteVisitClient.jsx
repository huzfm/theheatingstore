...
function LocationPicker({ value, onChange }) {
  ...
  /**
   * These two were one condition, `!MAPS_API_KEY || !mapsLoaded`, and the
   * branch below returns early on it. That meant every visitor saw the
   * no-API-key fallback, including its developer instruction to "add
   * NEXT_PUBLIC_GOOGLE_MAPS_API_KEY to .env.local", for the whole time the
   * Maps script was downloading, and the "Loading map..." overlay further
   * down was unreachable code.
   *
   * They are different states and now render differently: no key is a
   * permanent fallback to manual address entry, script-still-loading is a
   * loading state on the map itself.
   */
  const noMapsKey = !MAPS_API_KEY;
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      {/* Also disabled until the Maps script is in: the success callback
          reverse-geocodes through window.google, which does not exist yet, so
          an early press resolved to nothing at all. */}
      <button
        onClick={handleGPS}
        disabled={gpsLoading || !mapsLoaded}
        type="button"
        style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, padding: '10px 16px', borderRadius: 12, border: '1.5px solid rgba(196,98,58,0.25)', background: 'rgba(196,98,58,0.06)', color: '#C4623A', fontSize: 13, fontWeight: 600, cursor: gpsLoading || !mapsLoaded ? 'not-allowed' : 'pointer', transition: 'all 0.2s', opacity: gpsLoading || !mapsLoaded ? 0.6 : 1 }}
      >
        {gpsLoading ? (
          <>
            <Spinner size={14} strokeWidth={4} />
            Detecting location...
          </>
        ) : (
          <>
      ...
      <div style={{ position: 'relative', borderRadius: 16, overflow: 'hidden', border: '1px solid rgba(184,107,69,0.15)', height: 210 }}>
        {!mapsLoaded && (
          <div role="status" aria-busy="true" style={{ position: 'absolute', inset: 0, zIndex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#FFF8F0' }}>
            <div style={{ textAlign: 'center', color: '#B86B45' }}>
...
function PrimaryButton({ children, onClick, disabled, loading }) {
  ...
  return (
    ...
    >
      {/* Both states share a grid cell so the button keeps the width of its
          idle label instead of resizing to "Submitting..." mid-request. */}
      <PendingLabel
        pending={!!loading}
        idle={children}
        busy={
...
// ── Main Component ────────────────────────────────────────────────────────────
export default function BookSiteVisitClient() {
  ...
  const SUBFLOORS = [
    {
      ...
      chipColor: '#8B3A2A',
      img: '/images/stock/1504307651254-35680f356dfd-1600.webp',
    },
    {
      id: 'timber',
      label: 'Timber',
                ...
                {step === 1 && (
                  <>
                    ...
                    <div className="room-grid">
                      {[
                        { id: 'living', label: 'Living Room', img: '/images/stock/1555041469-a586c61ea9bc-500.webp' },
                        { id: 'kitchen', label: 'Kitchen', img: '/images/stock/1556909114-f6e7ad7d3136-500.webp' },
                        { id: 'bathroom', label: 'Bathroom', img: '/images/stock/1552321554-5fefe8c9ef14-500.webp' },
                        { id: 'bedroom', label: 'Bedroom', img: '/images/stock/1505693314120-0d443867891c-500.webp' },
                        { id: 'lobby', label: 'Lobby', img: '/images/stock/1600607687939-ce8a6c25118c-1600.webp' },
                      ].map((room, i) => (
                        <motion.div
                          key={room.id}
