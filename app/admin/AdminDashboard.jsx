...
function StatCard({ label, value, sub, color }) {
	return (
		<div style={{ background: 'rgba(255,255,255,0.72)', backdropFilter: 'blur(28px)', WebkitBackdropFilter: 'blur(28px)', border: '1px solid rgba(255,255,255,0.5)', borderRadius: 22, padding: '28px 24px', boxShadow: '0 8px 32px rgba(60,42,37,0.07)' }}>
			<p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 10, fontWeight: 600, letterSpacing: '0.2em', textTransform: 'uppercase', color: '#6B4A2D', marginBottom: 12 }}>{label}</p>
			<p style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 'clamp(2rem, 4vw, 3rem)', fontWeight: 700, color: color, lineHeight: 1, marginBottom: 6 }}>{value}</p>
			<p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 12, color: '#6B4A2D', lineHeight: 1.5 }}>{sub}</p>
		</div>
	);
}
	...
	if (active && payload && payload.length) {
		return (
			<div style={{ background: 'rgba(20,10,8,0.92)', backdropFilter: 'blur(16px)', borderRadius: 12, padding: '10px 16px', border: '1px solid rgba(245,185,122,0.3)' }}>
				<p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 13, fontWeight: 600, color: '#F5B97A' }}>{payload[0].payload.name}</p>
				<p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 11, color: 'rgba(255,255,255,0.7)' }}>{payload[0].value} selections</p>
			</div>
		);
	}
	return null;
...
export default function AdminDashboard() {
	...
	return (
		<>
			<style>{`
				.adm-page { min-height: 100vh; background: linear-gradient(180deg,#FFFFFF 0%,#FFF4E8 50%,#FFE8D0 100%); font-family: 'DM Sans', sans-serif; }
			...
			<div className='adm-page'>
				<div className='adm-content'>
					...
					<div className='adm-header'>
						<div>
							<h1 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 'clamp(1.5rem, 3vw, 2.25rem)', fontWeight: 700, color: '#2C1810', margin: 0, lineHeight: 1.1 }}>
								Installation Analytics
							</h1>
							<p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 13, color: '#6B4A2D', marginTop: 6 }}>
						...
						<button
							onClick={fetchData}
							style={{ padding: '10px 20px', borderRadius: 999, border: '1px solid rgba(184,107,69,0.3)', background: 'rgba(255,255,255,0.7)', fontFamily: "'DM Sans', sans-serif", fontSize: 12, fontWeight: 600, color: '#8B3A2A', cursor: 'pointer' }}>
							Refresh Data
						</button>
					</div>
					...
					<div className='adm-grid'>
						...
						<motion.div variants={fadeUp} initial='hidden' animate='show' className='adm-card' style={{ padding: '28px 28px 20px' }}>
							<h3 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 18, fontWeight: 600, color: '#2C1810', marginBottom: 4 }}>Size Demand Distribution</h3>
							<p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 12, color: '#6B4A2D', marginBottom: 24 }}>Number of times each preset was selected in the cost calculator</p>
							<div style={{ height: 280 }}>
								<ResponsiveContainer width='100%' height='100%'>
									<BarChart data={sizes} margin={{ top: 5, right: 10, left: -20, bottom: 5 }}>
										<XAxis dataKey='name' tick={{ fontFamily: "'DM Sans', sans-serif", fontSize: 12, fill: '#6B4A2D' }} axisLine={false} tickLine={false} />
										<YAxis tick={{ fontFamily: "'DM Sans', sans-serif", fontSize: 11, fill: '#6B4A2D' }} axisLine={false} tickLine={false} />
						...
						<motion.div variants={fadeUp} initial='hidden' animate='show' transition={{ delay: 0.1 }} className='adm-card' style={{ padding: '28px 28px 20px' }}>
							<h3 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 18, fontWeight: 600, color: '#2C1810', marginBottom: 4 }}>Market Share by Size</h3>
							<p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 12, color: '#6B4A2D', marginBottom: 24 }}>Percentage breakdown of size selections</p>
							<div style={{ height: 280 }}>
								<ResponsiveContainer width='100%' height='100%'>
									<PieChart>
							...
							<div style={{ display: 'flex', gap: 16, justifyContent: 'center', flexWrap: 'wrap', marginTop: 8 }}>
								{sizes.map((s) => (
									<div key={s.name} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
										...
										<span style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 11, color: '#6B4A2D' }}>
					...
					<motion.div variants={fadeUp} initial='hidden' animate='show' transition={{ delay: 0.2 }} className='adm-card' style={{ padding: '28px', marginTop: 24 }}>
						<h3 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 18, fontWeight: 600, color: '#2C1810', marginBottom: 20 }}>Recent Installation Inquiries</h3>
						{loading ? (
							<div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '20px 0' }}>
								...
								<span style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 13, color: '#6B4A2D' }}>Loading inquiries...</span>
							</div>
						) : leads.length === 0 ? (
							<div style={{ padding: '20px 0', textAlign: 'center' }}>
								<p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 13, color: '#6B4A2D' }}>No leads data available. Connect to the backend API to track inquiries.</p>
							</div>
						) : (
									...
									<thead>
										<tr style={{ borderBottom: '1px solid rgba(184,107,69,0.12)' }}>
											{['Name', 'Phone', 'Location', 'Source', 'Date'].map((h) => (
												<th key={h} style={{ padding: '10px 12px', fontFamily: "'DM Sans', sans-serif", fontSize: 10, fontWeight: 600, letterSpacing: '0.15em', textTransform: 'uppercase', color: '#6B4A2D', textAlign: 'left' }}>{h}</th>
											))}
										</tr>
									</thead>
									<tbody>
										{leads.map((lead, i) => (
											<tr key={i} style={{ borderBottom: '1px solid rgba(184,107,69,0.06)' }}>
												<td style={{ padding: '12px 12px', fontFamily: "'DM Sans', sans-serif", fontSize: 13, fontWeight: 600, color: '#2C1810' }}>{lead.name || '—'}</td>
												<td style={{ padding: '12px 12px', fontFamily: "'DM Sans', sans-serif", fontSize: 13, color: '#6B4A2D' }}>{lead.phone || '—'}</td>
												<td style={{ padding: '12px 12px', fontFamily: "'DM Sans', sans-serif", fontSize: 13, color: '#6B4A2D' }}>{lead.location || '—'}</td>
												<td style={{ padding: '12px 12px' }}>
													<span className='adm-badge' style={{ background: 'rgba(79,163,209,0.12)', color: '#4FA3D1' }}>{lead.source || 'Form'}</span>
												</td>
												<td style={{ padding: '12px 12px', fontFamily: "'DM Sans', sans-serif", fontSize: 12, color: '#6B4A2D' }}>
					...
					<div style={{ marginTop: 24, textAlign: 'center' }}>
						<p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 11, color: '#6B4A2D', opacity: 0.6 }}>
