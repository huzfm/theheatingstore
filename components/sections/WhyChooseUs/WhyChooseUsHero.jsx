...
/**
 * Opener for /why-choose-us, on the same recipe as every other route's hero.
 * The figures under the CTAs are the first three of the STATS list rather than
 * a separate set, so the hero cannot drift from the section below it.
 */
export default function WhyChooseUsHero() {
  const reduce = useReducedMotion();
  const facts = STATS.list.slice(0, 3);
    ...
    <section className="relative isolate overflow-hidden bg-ink-950 text-bone-100">
      ...
      <div className="relative z-10 mx-auto flex min-h-[100svh] max-w-5xl flex-col justify-center px-5 pb-16 pt-24 text-center sm:px-8 sm:pb-24 sm:pt-28">
        ...
        <RevealText
          ...
          className="mx-auto mt-6 max-w-[16ch] font-serif text-[clamp(2rem,8vw,5.25rem)] leading-[1] tracking-[0.005em] text-bone-100 [&_span]:leading-[inherit] sm:mt-7 sm:leading-[0.96]"
        >
          {HERO.headline}
        </RevealText>
